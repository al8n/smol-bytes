...
Immutable shared byte buffer. Mirrors `smol_bytes.shared::Bytes`, storing up to 62 bytes inline
before falling back to heap storage with reference-counted sharing. All `Buf` getters (e.g.
`get_u16_le`) are available, along with rich comparisons, slicing, and zero-copy split helpers."#;
...
/// Python bindings for the allocation-sharing [`shared::Bytes`](crate::shared::Bytes).
///
...
/// splitting, and comparison semantics that mirror `shared::Bytes`.
...
impl PySharedBytes {
  ...
  fn __repr__(&self) -> PyResult<String> {
    py_check_alloc(self.inner.len().saturating_mul(6).saturating_add(64))?;
  ...
  ///
  /// Returns a `(constructor, args)` tuple so that
  /// `pickle.loads(pickle.dumps(b))` reconstructs the object.
