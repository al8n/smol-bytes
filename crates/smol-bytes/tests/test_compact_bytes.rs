...
use smol_bytes::{compact::Bytes, Buf, BufMut, BytesMut};
...
fn freeze_clone_unique() {
  let s = &b"abcdefgh"[..];
  let b = BytesMut::from(s).freeze_compact();
  assert_eq!(b, s);
  let c = b.clone();
  assert_eq!(c, s);
}
...
fn test_bytes_into_vec() {
  ...
  eprintln!("3");
  let b1 = bytes_mut.freeze_compact();
  eprintln!("4");
  let b2 = b1.clone();
  ...
  let b1 = bytes_mut1.freeze_compact();
  let b2 = bytes_mut2.freeze_compact();
...
fn mut_shared_is_unique() {
  ...
  b.make_heap();
  let c = b.split().unwrap().freeze_compact();
  assert!(!c.is_unique());
  drop(b);
  assert!(c.is_unique());
}
...
fn test_bytesmut_from_bytes_bytes_mut_vec() {
  ...
  let mut bytes_mut: BytesMut = bs[..].into();
  bytes_mut = BytesMut::from(bytes_mut.freeze_compact());
  assert_eq!(bytes_mut, bs[..]);
...
fn try_reclaim_arc() {
  ...
  buf.put_slice(b"abc");
  buf.make_heap();
  let x = buf.split().unwrap().freeze_compact();
  buf.put_slice(b"def");
  // Reclaiming a ludicrous amount of space should calmly return false
  assert!(!buf.try_reclaim(usize::MAX));

  let y = buf.split().unwrap().freeze_compact();
  let z = y.clone();
  assert!(!buf.try_reclaim(6));
  drop(x);
  drop(z);
...
// Uncomment these tests when `slice_ref` is implemented.
